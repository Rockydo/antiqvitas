...
def validate(records: tuple[Current, ...]) -> None:
    ...
    if missing_images:
        raise ValueError(f"M10 third-century current(s) lack reviewed event illustrations: {missing_images}")
...
def event_script(records: tuple[Current, ...]) -> str:
    ...
    for record in records:
        ...
        lines.extend((
            ...
            *impact_lines(record),
            *knowledge_response_lines(record.kind, 2),
            "\t}",
            "}",
            "",
        ))
...
def coas() -> str:
    return "\n".join((
        "# Generated by tools/m10_third_century.py --write; M11 non-reconstructive formation UI standards.",
        "ALM = {",
        "\tpattern = \"pattern_solid.dds\"",
        "\tcolor1 = \"green\"",
        "\tcolor2 = \"yellow\"",
        "\tcolor3 = \"white\"",
        "\tcolored_emblem = { texture = \"ce_boar_passant.dds\" color1 = color2 color2 = color2 color3 = color3 instance = { position = { 0.5 0.52 } scale = { 0.82 0.70 } } }",
        "}",
        "",
        "SAS = {",
        "\tpattern = \"pattern_solid.dds\"",
