...
def market_manager() -> tuple[str, int]:
    ...
    A hub may retain its historical/urban data while deferring an engine startup
    seed where the installed market manager asserts at the first monthly pulse.
    """
    ...
    for row in entries:
        ...
        startup_mode = (row.get("startup_mode") or "deferred").strip()
        if startup_mode != "deferred":
    ...
    lines = [
        ...
        "# All source-led hubs retain their urban and harbor data, but the installed",
        "# pre-game market manager asserts on the AD 1 setup; see docs/DECISIONS.md.",
        "market_manager = {",
    ]
    for row in sorted(entries, key=lambda item: item["key"]):
        lines.append(
            f"\t# deferred_market = {row['location']} # {row['name']}; {row['source']}"
        )
    lines.extend(("}", ""))
