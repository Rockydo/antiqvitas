...
def country_manager() -> tuple[str, int]:
    """Render the first M3 country slice from verified capital locations only.
    ...
    lines = [
        ...
        "# M3 capital-control slice; extend from sourced ownership data only.",
    ...
    for row in rows:
        ...
        lines.extend(
            (
                f'\t\t{tag_map[row["tag"]]} = {{ # {row["name"]}; {row["source"]}',
                "\t\t\town_control_core = {",
                f"\t\t\t\t{capital}",
                "\t\t\t}",
                "",
...
def generated_files() -> tuple[dict[str, str], int]:
    countries, count = country_manager()
    return {**STATIC_FILES, "10_countries.txt": countries}, count
...
def main() -> int:
    ...
    failures: list[str] = []
    files, country_count = generated_files()
    if args.write:
    ...
    print(
        ...
        f"{country_count} verified-capital countries)"
    )
    return 0
