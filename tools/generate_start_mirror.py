...
# `enable_religion` contract and therefore need no ahistorical opening pop.
OPENING_RELIGION_SEEDS = {
    "axum": (
        "antq_aksumite_paganism", "antq_kushite_amun", "antq_geez", "peasants",
    ),
    "daga_papua": (
        "antq_australian_dreaming", "antq_papuan_local_traditions",
        "antq_daga_highland", "peasants",
    ),
    "lhasa": (
        "antq_bon", "antq_central_plateau_traditions", "antq_central_tsangpo",
        "tribesmen",
    ),
    "patna": (
        "antq_jainism", "antq_brahmanism", "antq_magadhi_prakrit", "peasants",
    ),
    "alexandria": (
        "antq_kemetic", "antq_religio_romana", "antq_egyptian", "peasants",
    ),
    "tunis": (
        "antq_punic", "antq_religio_romana", "antq_phoenician_punic", "peasants",
    ),
    "surgut": (
        "antq_siberian", "antq_finnic", "antq_kulay", "tribesmen",
    ),
}
...
def compatibility_presence_manager(cultures: list[str]) -> str:
    """Render engine-only culture compatibility presences."""
    lines = [
        "# Generated by tools/generate_start_mirror.py --write.",
...
def population_manager(
    ...
    for location in sorted(owners):
        ...
        if opening_seed:
            ...
            lines.append(
                f"\t\tdefine_pop = {{ type = {seed_type} "
                f"size = {RELIGION_RETENTION_POP_SIZE:.3f} "
                f"culture = {seed_culture} religion = {seed_religion} }} "
                "# Bounded opening community; total-neutral."
            )
        lines.append("\t}")
    lines.extend(("}", ""))
    return (
