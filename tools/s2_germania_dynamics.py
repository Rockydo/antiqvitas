...
AI_PULSE_OUTPUT = ROOT / "in_game/common/on_action/antq_s2_germania_ai_pulse.txt"
...
AI_ACTION_HOSTS = (
    "XAA", "XBK", "CRU", "XBL", "XBM", "BTV",
    "XBM", "XBN", "SEM", "GUT", "XBR", "AES",
)
...
def action_script() -> str:
    return "# Generated by tools/s2_germania_dynamics.py --write.\n\n" + "\n\n".join(
        action_block(action) for action in ACTIONS
    ) + "\n"
...
def ai_pulse_script() -> str:
    lines = [
        ...
        "\ttrigger = {",
        "\t\tis_ai = yes",
        "\t\tOR = { " + " ".join(f"tag = {tag}" for tag in sorted(set(AI_ACTION_HOSTS))) + " }",
        "\t}",
        "\teffect = {",
    ]
    for month, (action, host) in enumerate(zip(ACTIONS, AI_ACTION_HOSTS, strict=True), start=1):
        cooldown = f"{action.key}_ai_cooldown"
        lines.extend((
            "\t\tif = {",
            "\t\t\tlimit = {",
            f"\t\t\t\ttag = {host}",
            ...
            f"\t\t\t\texists = international_organization:{action.io}",
            f"\t\t\t\tis_member_of_international_organization = international_organization:{action.io}",
            f"\t\t\t\tinternational_organization:{action.io} = {{ total_members > 1 }}",
            f"\t\t\t\tNOT = {{ has_variable = {cooldown} }}",
            "\t\t\t}",
            "\t\t\tsave_scope_as = antq_s2_germania_ai_actor",
            f"\t\t\tadd_gold = -{action.cost}",
        ))
        if action.actor_effect:
            lines.append(f"\t\t\t{action.actor_effect}")
        lines.extend((
            f"\t\t\tset_variable = {{ name = {cooldown} value = yes years = 5 }}",
            f"\t\t\tinternational_organization:{action.io} = {{",
            "\t\t\t\trandom_international_organization_member = {",
            "\t\t\t\t\tlimit = { this != scope:antq_s2_germania_ai_actor }",
            f"\t\t\t\t\tadd_opinion = {{ target = scope:antq_s2_germania_ai_actor modifier = {action.opinion} }}",
        ))
        if action.target_effect:
            lines.append(f"\t\t\t\t\t{action.target_effect}")
        lines.extend(("\t\t\t\t}", "\t\t\t}", "\t\t}",))
    lines.extend(("\t}", "}", ""))
    return "\n".join(lines)
...
def outputs() -> dict[Path, str]:
    rendered = {
        ...
        AI_PULSE_OUTPUT: ai_pulse_script(),
...
def validate() -> list[str]:
    ...
    for forbidden in (
        ...
        if forbidden in action_text:
            failures.append(f"Germania actions expose unsafe generic-action AI token: {forbidden}")
    pulse_text = outputs()[AI_PULSE_OUTPUT]
    if pulse_text.count("\t\tif = {") != len(ACTIONS):
        failures.append("Germania direct AI action calendar is incomplete")
    if any(f"current_month = {month}" not in pulse_text for month in range(1, 13)):
        failures.append("Germania direct AI action calendar is not collision-free")
    if pulse_text.count(f"gold >= {AI_TREASURY_RESERVE}") != len(ACTIONS):
        failures.append("Germania direct AI actions do not preserve the treasury reserve")
    if outputs()[AI_LIST_OUTPUT].count("potential = { always = no }") != 1:
        failures.append("Germania generic-action AI registry is not hard-disabled")
