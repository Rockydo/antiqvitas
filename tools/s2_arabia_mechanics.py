...
AI_PULSE_OUTPUT = ROOT / "in_game/common/on_action/antq_s2_arabian_route_ai_pulse.txt"
...
AI_ACTION_HOSTS = ("NAB", "HIM", "QAT", "KIN")
...
def ai_pulse_script() -> str:
    lines = [
        ...
        "\ttrigger = {",
        "\t\tis_ai = yes",
        "\t\tOR = { " + " ".join(f"tag = {tag}" for tag in AI_ACTION_HOSTS) + " }",
        "\t}",
        "\teffect = {",
    ]
    for month, action, host in zip((2, 5, 8, 11), ACTIONS, AI_ACTION_HOSTS, strict=True):
        ...
        lines.extend((
            ...
            f"\t\t\t\texists = international_organization:{IO_KEY}",
            f"\t\t\t\tis_member_of_international_organization = international_organization:{IO_KEY}",
            f"\t\t\t\tinternational_organization:{IO_KEY} = {{ total_members > 1 }}",
        ...
        lines.extend((
            f"\t\t\tset_variable = {{ name = {cooldown} value = yes years = 3 }}",
            f"\t\t\tinternational_organization:{IO_KEY} = {{",
            "\t\t\t\trandom_international_organization_member = {",
            "\t\t\t\t\tlimit = { this != scope:antq_s2_arabian_route_ai_actor }",
            f"\t\t\t\t\tadd_opinion = {{ target = scope:antq_s2_arabian_route_ai_actor modifier = {action.opinion} }}",
        ))
...
def validate() -> list[str]:
    ...
    action_text = outputs()[ACTION_OUTPUT]
    for forbidden in (
        "ai_tick", "automation_tick", "ai_prerequisite", "ai_will_do",
        "ai_interaction_source_list",
    ):
        if forbidden in action_text:
            failures.append(f"Arabian route actions expose unsafe generic-action AI token: {forbidden}")
    pulse_text = outputs()[AI_PULSE_OUTPUT]
    if pulse_text.count("\t\tif = {") != len(ACTIONS):
        failures.append("Arabian route direct AI action calendar is incomplete")
    if pulse_text.count(f"gold >= {AI_TREASURY_RESERVE}") != len(ACTIONS):
        failures.append("Arabian route direct AI actions do not preserve the treasury reserve")
    if outputs()[AI_LIST_OUTPUT].count("potential = { always = no }") != 1:
        failures.append("Arabian route generic-action AI registry is not hard-disabled")
