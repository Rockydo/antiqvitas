#!/usr/bin/env python3
"""Guard ANTIQVITAS's player-facing English against clear post-476 vocabulary.
...
audit examines values in the mod's English-first localization files. A
separate source-pinned generator owns the inherited mounted trait surface that
can reach the player without being authored in this repository. The M11 mirror
validator proves that every supported client receives the same values.
"""
...
ANCIENT_EXPANSION = ENGLISH_DIR / "zzzz_antq_ancient_expansion_l_english.yml"
REPORT = ROOT / "docs/m12/ANACHRONISM_AUDIT.md"
GENERATED_NOTICE = (
    "<!-- Generated by tools/m12_anachronism_audit.py; edit localization inputs "
...
def english_files() -> tuple[Path, ...]:
    files = tuple(sorted(ENGLISH_DIR.glob("*_l_english.yml")))
    if not files:
    ...
    for path in files:
        ...
        for number, line in enumerate(lines[1:], start=2):
            ...
            if match is None:
                ...
                continue
            entries += 1
            # The ancient expansion layer necessarily retains engine concept
            # identifiers such as [colonize|e] and scripted object names inside
            # non-rendered tokens. Its generator separately proves that none of
            # those terms survive in player-rendered prose.
            value = match.group("value")
            if path == ANCIENT_EXPANSION:
                from m12_ancient_expansion_loc import protected_prose

                value = protected_prose(value)
            forbidden = FORBIDDEN.search(value)
...
def report_bytes(files: tuple[Path, ...], entries: int) -> bytes:
    return render(files, entries).encode("utf-8")


def write() -> None:
    files, entries = audit()
    REPORT.parent.mkdir(parents=True, exist_ok=True)
    REPORT.write_bytes(report_bytes(files, entries))
...
def check() -> bool:
    try:
        files, entries = audit()
        expected = report_bytes(files, entries)
