#!/usr/bin/env python3
"""Validate the sourced AD 1 West African residual replacement."""
...
LEDGER = ROOT / "docs/m12/west_africa_granularity.csv"
LANGUAGES = (
...
EXPECTED = {
    ...
    "NSK": (
        "Nsukka-Lejja Ironworkers", "awka_etiti", 1, "antq_nsukka_lejja",
    ...
    "VIC": (
        "Volta-Cong Interfluve", "kong", 1, "antq_volta_basin",
        "antq_west_african_ironworking_network",
    ),
    "BSW": (
...
EXPECTED_AREAS = {
    ("WAF", "kebbi_province"),
    ("ZMP", "zamfara_province"),
    ("KPC", "katsina_province"),
    ("GTC", "gobir_province"),
}
EXPECTED_SINGLETONS = {
...
def expected_rows() -> tuple[list[dict[str, str]], list[str]]:
    ...
    if sum(counts[tag] for tag in EXPECTED) != 24:
        failures.append("reviewed replacement must control exactly 24 locations")
    for pair in EXPECTED_AREAS - areas:
        failures.append(f"missing reviewed province frame {pair[0]}:{pair[1]}")
    for pair in EXPECTED_SINGLETONS - locations:
        failures.append(f"missing reviewed singleton {pair[0]}:{pair[1]}")
    for geography in ("guinea_region", "sahel_region"):
        if ("WAF", geography) in residuals:
            failures.append(f"obsolete West-African residual remains: {geography}")

    ledger: list[dict[str, str]] = []
    for tag, expected in EXPECTED.items():
        name, capital, count, culture, reform = expected
        row = roster.get(tag)
        profile = profiles.get(tag)
        government = governments.get(tag)
        coa = coas.get(tag)
        if row is None:
            failures.append(f"missing reviewed West-African frame {tag}")
            continue
        if row["name"] != name:
            failures.append(f"{tag} must display as {name}, found {row['name']}")
        if re.search(r"\b(?:societies|land of|generic|placeholder)\b", row["name"], re.I):
        ...
        if profile is None:
            ...
            if profile["religion"] != "antq_west_african":
...
def main() -> int:
    ...
    try:
        ...
        if args.write and not failures:
            ...
            print(
                "s2_west_africa_granularity: wrote "
        ...
        if failures:
            print("s2_west_africa_granularity: FAIL")
            for failure in failures:
                print(f"  - {failure}")
            return 1
        print(
            "s2_west_africa_granularity: PASS "
            "(8 frames; 24 owned entries; largest 7; 8 cultures; "
            "3 reforms; 8 standards; 11-client localization)"
        )
        return 0
    except (OSError, ValueError, KeyError, json.JSONDecodeError) as exc:
        print(f"s2_west_africa_granularity: FAIL\n  - {exc}")
        return 1
