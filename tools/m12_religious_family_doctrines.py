...
OPENING_SCRIPT = ROOT / "in_game/common/on_action/antq_m12_opening_doctrines.txt"
ROSTER = ROOT / "docs/world_1ad/polities.csv"
TAG_MAP = ROOT / "docs/world_1ad/tag_map.json"
TAG_PROFILES = ROOT / "docs/m4/tag_profiles.csv"
REGIONAL_PROFILES = ROOT / "docs/m4/regional_profiles.csv"
ROMAN_LEDGER = ROOT / "docs/m12/religious_doctrines.csv"
...
def _profile_map(path: Path, key: str) -> dict[str, str]:
    with path.open(encoding="utf-8-sig", newline="") as handle:
        raw = list(csv.DictReader(handle))
    result = {row[key].strip(): row["religion"].strip() for row in raw}
    if len(result) != len(raw) or any(not value for value in result.values()):
        raise ValueError(f"{path.relative_to(ROOT)} has duplicate or blank profiles")
    return result


def _religious_aspect_slots() -> dict[str, int]:
    import re

    lines = RELIGIONS.read_text(encoding="utf-8-sig").splitlines()
    depth = 0
    ...
    for line in lines:
        code = line.split("#", 1)[0]
        if depth == 0:
            match = re.match(r"^\s*(antq_[A-Za-z0-9_]+)\s*=\s*\{", code)
            if match:
                current = match.group(1)
        elif current is not None and depth == 1:
            match = re.match(r"^\s*religious_aspects\s*=\s*([123])\s*$", code)
            if match:
                if current in slots:
                    raise ValueError(f"{current} repeats religious_aspects")
                slots[current] = int(match.group(1))
        depth += code.count("{") - code.count("}")
        if depth == 0:
            current = None
    if depth != 0:
        raise ValueError(f"{RELIGIONS.relative_to(ROOT)} has unbalanced braces")
    return slots


def opening_packages(
    doctrines: tuple[Doctrine, ...],
) -> tuple[tuple[str, str, tuple[str, ...]], ...]:
    choices: dict[str, list[str]] = {}
    for row in doctrines:
        choices.setdefault(row.religion, []).append(row.key)
    with ROMAN_LEDGER.open(encoding="utf-8-sig", newline="") as handle:
        roman_rows = list(csv.DictReader(handle))
    choices["antq_religio_romana"] = [row["key"].strip() for row in roman_rows]

    slots = _religious_aspect_slots()
    regions = _profile_map(REGIONAL_PROFILES, "region")
    tags = _profile_map(TAG_PROFILES, "tag")
    tag_payload = json.loads(TAG_MAP.read_text(encoding="utf-8"))
    engine_tags = {
        row["design_tag"]: row["engine_tag"] for row in tag_payload["entries"]
    }
    ...
    seen_engine_tags: set[str] = set()
    for row in roster:
        design_tag = row["tag"].strip()
        religion = tags.get(design_tag, regions.get(row["region"].strip(), ""))
        if religion not in slots or religion not in choices:
            raise ValueError(f"no opening doctrine package for {design_tag}/{religion}")
        engine_tag = engine_tags.get(design_tag, "")
        selected = tuple(choices[religion][: slots[religion]])
        if (
            not engine_tag
            or engine_tag in seen_engine_tags
            or len(selected) != slots[religion]
            or len(set(selected)) != len(selected)
        ):
            raise ValueError(f"invalid opening doctrine assignment for {design_tag}")
        packages.append((engine_tag, religion, selected))
        seen_engine_tags.add(engine_tag)
    if len(packages) != 463 or len(seen_engine_tags) != 463:
        raise ValueError("opening doctrines must cover exactly 463 active countries")
    return tuple(packages)


def render_opening_script(doctrines: tuple[Doctrine, ...]) -> bytes:
    lines = [
        "# Generated by tools/m12_religious_family_doctrines.py --write.",
        "# Complete faith-specific opening packages avoid EU5's multi-target doctrine AI race.",
        "on_game_start = {",
        "\ton_actions = { antq_m12_seed_opening_doctrines }",
        "}",
        "",
        "antq_m12_seed_opening_doctrines = {",
        "\teffect = {",
    ]
    for engine_tag, religion, selected in opening_packages(doctrines):
        lines.append(f"\t\tc:{engine_tag} = {{ # {religion}")
        lines.extend(
            f"\t\t\tadd_religious_aspect = religious_aspect:{key}"
            for key in selected
        )
        lines.append("\t\t}")
    lines.extend(("\t}", "}"))
...
def write() -> None:
    ...
    OPENING_SCRIPT.parent.mkdir(parents=True, exist_ok=True)
    OPENING_SCRIPT.write_bytes(render_opening_script(doctrines))
...
def check() -> bool:
    try:
        ...
        if (
            not OPENING_SCRIPT.is_file()
            or OPENING_SCRIPT.read_bytes() != render_opening_script(doctrines)
        ):
            errors.append(f"stale or missing {OPENING_SCRIPT.relative_to(ROOT)}")
        for language in LANGUAGES:
            target = localization_path(language)
            if (
                not target.is_file()
