...
def build_report() -> dict[str, object]:
    ...
    require(len(subjects) == 35, f"expected 35 start dependencies, got {len(subjects)}", failures)
    require(
    ...
    require(
        len(political_contracts) == 184
        and len({row["reform"] for row in political_contracts}) == 184
        and len({row["modifiers"] for row in political_contracts}) >= 180,
    ...
    require(
        ...
        and sum(count == 2 for count in alternative_profiles.values()) == 37
        and len(alternative_profiles) == 45,
    ...
    require(
        len(law_profiles) == 348
        and len({row["tag"] for row in law_profiles}) == 348
    ...
    require(
        councils.count("parliament_base_support =") == 45
        and councils.count("_agenda_impact =") == 135,
