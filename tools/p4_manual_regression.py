...
def build_report() -> dict[str, object]:
    ...
    require(
        len(political_contracts) == 203
        and len({row["reform"] for row in political_contracts}) == 203
        and len({row["modifiers"] for row in political_contracts}) >= 198,
    ...
    require(
        len(law_profiles) == 380
        and len({row["tag"] for row in law_profiles}) == 380
