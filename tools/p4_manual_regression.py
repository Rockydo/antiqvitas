...
def build_report() -> dict[str, object]:
    ...
    require(
        len(estate_orders) == 198 and len({row["key"] for row in estate_orders}) == 198,
    ...
    require(
        len(political_contracts) == 93
        and len({row["reform"] for row in political_contracts}) == 93
        and len({row["modifiers"] for row in political_contracts}) >= 80,
    ...
    require(
        len(alternative_reforms) == 58
        and set(alternative_profiles.values()) == {2}
        and len(alternative_profiles) == 29,
    ...
    require(
        councils.count("parliament_base_support =") == 29
        and councils.count("_agenda_impact =") == 87,
