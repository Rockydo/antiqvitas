...
def estate_privileges(data: PowerData) -> str:
    ...
    # A start privilege must pass its own potential after every regional
    # government overlay has been applied.  Derive the exact opening-reform
    # union from the resolved country profiles so later regional additions
    # cannot silently produce "Removing invalid estate privilege" at game start.
    assigned_reforms: dict[str, set[str]] = {}
    for government in data.governments.values():
        for privilege in pipe_values(
            government["privileges"],
            f"government {government['design_tag']} opening privileges",
        ):
            assigned_reforms.setdefault(privilege, set()).add(government["reform"])
    for row in data.privileges:
        ...
        opening_reforms = sorted(
            assigned_reforms.get(row["key"], set()) - set(declared_reforms)
        )
        if declared_reforms or opening_reforms or row["potential_tags"]:
            lines.extend(("\tpotential = {", "\t\tOR = {"))
            lines.extend(
                f"\t\t\thas_reform = government_reform:{reform}"
                for reform in (*declared_reforms, *opening_reforms)
            )
            if row["potential_tags"]:
                tags = pipe_values(
