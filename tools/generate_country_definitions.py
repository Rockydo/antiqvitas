...
DIRECT_COAS = ROOT / "docs/m11/coa_direct_catalog.csv"
...
def load_coa_overrides() -> dict[str, CoaOverride]:
    ...
    with DIRECT_COAS.open(encoding="utf-8-sig", newline="") as handle:
        rows = list(csv.DictReader(handle))
    overrides: dict[str, CoaOverride] = {}
    for row in rows:
        missing = [field for field in required if not row.get(field, "").strip()]
        if missing:
            raise ValueError(f"{DIRECT_COAS.relative_to(ROOT)} has blank fields: {', '.join(missing)}")
        tag = row["tag"].strip()
        if tag in overrides:
            raise ValueError(f"{DIRECT_COAS.relative_to(ROOT)} repeats tag {tag}")
...
def validate_references() -> list[str]:
    ...
    if unknown_coa_tags:
        failures.append(f"M11 direct CoAs have unknown roster tags: {', '.join(unknown_coa_tags)}")
    missing_coa_tags = sorted(roster_tags - set(coa_overrides))
    if missing_coa_tags:
        failures.append(f"M11 direct CoAs are missing roster tags: {', '.join(missing_coa_tags)}")
