...
def placeholder_coas(rows: list[dict[str, str]], tags: dict[str, str]) -> str:
    ...
    lines = [
        "# Generated by tools/generate_country_definitions.py --write.",
        "# M3 placeholder flags plus bounded M11 core emblem standards.",
    ]
    for row in rows:
        profile = profile_for(row)
        override = overrides.get(row["tag"])
        if override is not None:
            lines.extend(
                (
                    "",
                    f'{tags[row["tag"]]} = {{ # {row["name"]}; {override.source}',
                    '\tpattern = "pattern_solid.dds"',
                    f'\tcolor1 = "{override.color1}"',
                    f'\tcolor2 = "{override.color2}"',
                    f'\tcolor3 = "{override.color3}"',
                    "\tcolored_emblem = {",
                    f'\t\ttexture = "{override.emblem}"',
                    f'\t\tcolor1 = {override.emblem_color1}',
                    f'\t\tcolor2 = {override.emblem_color2}',
                    f'\t\tcolor3 = {override.emblem_color3}',
                    f'\t\tinstance = {{ position = {{ {override.position_x} {override.position_y} }} scale = {{ {override.scale_x} {override.scale_y} }} }}',
                    "\t}",
                    "}",
                )
            )
...
def validate_references() -> list[str]:
    ...
    for tag, override in coa_overrides.items():
        if override.confidence not in {"secure", "contested"}:
            failures.append(f"M11 core CoA {tag}: invalid confidence {override.confidence}")
        if not override.emblem.startswith("ce_") or not override.emblem.endswith(".dds"):
            failures.append(f"M11 core CoA {tag}: invalid colored-emblem name {override.emblem}")
        if not (colored_emblems / override.emblem).is_file():
            failures.append(f"M11 core CoA {tag}: absent local colored emblem {override.emblem}")
    for row in rows:
        profile = profile_for(row)
