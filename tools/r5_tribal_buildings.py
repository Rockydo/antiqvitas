...
def upkeep(entry):
    if entry["produced"]:
        ...
        # Household labour and retained seed/herds are intrinsic to tribesmen
        # employment.  Monetize only bounded tool and repair-material overhead;
        # the former generic livestock share made pastoral outputs consume
        # themselves and amplified a local shortage into sovereign bankruptcy.
        implement_good="leather" if entry["produced"]=="tools" else "tools"
        return [(implement_good,target*.55/prices[implement_good]),("lumber",target*.45/prices["lumber"])]
...
def building_text(entries):
    ...
    build={
        "subsistence":("rural_build_time",TRIBAL_CONSTRUCTION_PACKAGES["subsistence"]),
        "pastoral":("rural_build_time",TRIBAL_CONSTRUCTION_PACKAGES["pastoral"]),
        "craft":("workshop_build_time",TRIBAL_CONSTRUCTION_PACKAGES["craft"]),
        "exchange":("village_build_time",TRIBAL_CONSTRUCTION_PACKAGES["exchange"]),
        "ritual":("cultural_building_time",TRIBAL_CONSTRUCTION_PACKAGES["ritual"]),
        "warrior":("small_fort_building",TRIBAL_CONSTRUCTION_PACKAGES["warrior"]),
    }
    for e in entries:
        key=e["key"]; bt,demand=build[e["branch"]]
        lines += [f"{key} = {{","\taudio_tier = 1","\tis_foreign = no","\tpop_type = tribesmen",f"\tcategory = {category[e['branch']]}","\temployment_size = generic_tribesmen_employment","\trural_settlement = yes","\ttown = yes","\tcity = yes","\tmegalopolis = yes",f"\tbuild_time = {bt}",f"\tmax_levels = {e['max_levels']}","\tcountry_potential = {","\t\thas_tribal_government = yes",*ai_capital_affordability_trigger(),"\t}","\tallow = {","\t\talways = yes","\t}","\tlocation_potential = {",*gate_lines(e["gate"]),*profile_lines(e),"\t}","\tunique_production_methods = {",f"\t\t{key}_maintenance = {{"]
...
def seed_rows(entries):
    ...
    # bookmark placements. Seed a compact terrain-neutral package spanning
    # subsistence, craft, storage/exchange, and defence; the remaining shared
    # and regional families stay available as construction choices.
    opening=(
...
def check(entries):
    ...
    expected_branch_costs = {
        "subsistence": (("lumber", "0.05"), ("tools", "0.01")),
        "pastoral": (("lumber", "0.06"), ("livestock", "0.03"), ("leather", "0.01")),
        "craft": (("lumber", "0.08"), ("tools", "0.02"), ("clay", "0.03")),
        "exchange": (("lumber", "0.04"), ("pottery", "0.02")),
        "ritual": (("lumber", "0.08"), ("cloth", "0.02"), ("pottery", "0.02")),
        "warrior": (("lumber", "0.15"), ("weaponry", "0.02"), ("leather", "0.03")),
    }
    for branch, expected in expected_branch_costs.items():
        package = TRIBAL_CONSTRUCTION_PACKAGES.get(branch, "")
        if PACKAGE_GOODS.get(package) != expected:
            failures.append(f"{branch} tribal construction basket drift")
    ...
    prices=good_prices()
    for e in entries:
        if not e["produced"]: continue
        inputs=upkeep(e)
        input_goods={good for good,_amount in inputs}
        if e["produced"] in input_goods: failures.append(f"self-consuming tribal recipe {e['key']}")
        gross=prices[e["produced"]]*float(e["output"])
        cost=sum(prices[good]*amount for good,amount in inputs)
        if abs(cost - gross/1.20) > 1e-9: failures.append(f"tribal rural profit contract drift {e['key']}")
    if building_text(entries).count("\t\thas_tribal_government = yes") != len(entries): failures.append("every tribal building must be isolated to tribal and steppe governments")
    rendered=building_text(entries)
    if rendered.count("\tcountry_potential = {") != len(entries): failures.append("tribal country-affordability gate coverage drift")
    if rendered.count("\tallow = {") != len(entries): failures.append("tribal live affordability gate coverage drift")
    if rendered.count("\n".join(ai_capital_affordability_trigger())) != len(entries): failures.append("tribal country-affordability gate contract drift")
    if rendered.count("\tallow = {\n\t\talways = yes\n\t}") != len(entries): failures.append("tribal live construction allow must be player-reachable")
    ...
    if len(counts)!=337 or set(counts.values())!={4}: failures.append(f"SoP opening tribal placement coverage drift: tags={len(counts)} counts={sorted(set(counts.values()))}")
