...
AI_LIST_OUTPUT = ROOT / "in_game/common/generic_action_ai_lists/antq_m11_decisions_list.txt"
...
def decision_ai_months(decisions: list[Decision]) -> dict[str, int]:
    """Assign overlapping tag rosters to distinct monthly AI slots.

    EU5 releases generic-action AI after its 36-month opening phase. Evaluating
    several gold-spending decisions for one country in the same monthly plan lets
    the first command change the treasury and makes later queued commands invalid.
    Conflict coloring preserves every AI decision while ensuring that a country
    considers at most one M11 decision in any calendar month.
    """
    tag_sets = {row.identifier: set(row.tags) for row in decisions}
    conflicts = {
        row.identifier: {
            other.identifier
            for other in decisions
            if other.identifier != row.identifier
            and tag_sets[row.identifier] & tag_sets[other.identifier]
        }
        for row in decisions
    }
    order = sorted(
        decisions,
        key=lambda row: (-len(conflicts[row.identifier]), row.identifier),
    )
    months: dict[str, int] = {}
    for row in order:
        unavailable = {
            months[key]
            for key in conflicts[row.identifier]
            if key in months
        }
        month = next(
            (value for value in range(1, 13) if value not in unavailable),
            None,
        )
        if month is None:
            raise ValueError(
                f"{row.identifier}: decision overlap needs more than 12 AI months"
            )
        months[row.identifier] = month
    for tag in sorted({tag for row in decisions for tag in row.tags}):
        tag_months = [
            months[row.identifier] for row in decisions if tag in row.tags
        ]
        if len(tag_months) != len(set(tag_months)):
            raise ValueError(f"{tag}: overlapping decisions share an AI month")
    return months


def render_action(
    decision: Decision, engines: dict[str, str], ai_month: int
) -> str:
...
def outputs(decisions: list[Decision], engines: dict[str, str]) -> dict[Path, str]:
    ai_months = decision_ai_months(decisions)
    rendered = "# Generated by tools/m11_decisions.py from docs/m11/decisions.csv.\n\n"
    rendered += "\n".join(
        render_action(
            decision, engines, ai_months[decision.identifier]
        ).replace(
            "\tai_tick = monthly\n",
            "\tai_tick = never\n"
            "\tai_prerequisite = {\n"
            f"\t\tcurrent_month = {ai_months[decision.identifier]}\n"
    ...
    action_rows = "\n".join(f"\t\tantq_{decision.identifier}" for decision in decisions)
    ai_list = (
        "# Generated by tools/m11_decisions.py; explicit engine AI registry.\n"
        "antq_m11_decisions_list = {\n"
    ...
    paths = {OUTPUT: rendered, AI_LIST_OUTPUT: ai_list}
...
def check(decisions: list[Decision], engines: dict[str, str]) -> bool:
    ...
    rendered = outputs(decisions, engines)[OUTPUT]
    if rendered.count("ai_tick = never") != len(decisions):
        failures.append("not every decision delegates scheduling to its AI list")
    if "ai_tick = monthly" in rendered or "add = -1000" in rendered:
        failures.append("obsolete direct-tick or player-only isolation survived")
    if rendered.count("automation_tick = never") != len(decisions):
        failures.append("not every decision keeps player automation opt-in disabled")
    ai_months = decision_ai_months(decisions)
    if rendered.count("ai_prerequisite = {") != len(decisions):
        failures.append("not every decision has one conflict-free AI month")
    if rendered.count(f"gold >= {AI_TREASURY_RESERVE}") != len(decisions):
        failures.append("not every AI decision preserves the shared treasury reserve")
    if max(ai_months.values()) > 12:
        failures.append("decision AI schedule exceeds the calendar")
    ai_registry = outputs(decisions, engines)[AI_LIST_OUTPUT]
    if "potential = { always = yes }" not in ai_registry:
        failures.append("M11 decision AI list is not autonomously available")
    for decision in decisions:
        if ai_registry.count(f"antq_{decision.identifier}") != 1:
            failures.append(f"{decision.identifier} lacks one explicit AI-list registration")
    for decision in decisions:
        for good, threshold, _supply in decision_market_rows(decision.identifier):
            block = f"goods = goods:{good}\n\t\t\t\t\tamount = -{threshold}"
            if block not in rendered:
                failures.append(f"{decision.identifier} does not consume gated {good}")
    if failures:
