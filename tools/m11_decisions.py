...
def check(decisions: list[Decision], engines: dict[str, str]) -> bool:
    ...
    if rendered.count("ai_tick = monthly") != len(decisions):
        failures.append("not every decision has monthly AI evaluation")
    if "ai_tick = never" in rendered or "add = -1000" in rendered:
        failures.append("player-only decision gating survived")
