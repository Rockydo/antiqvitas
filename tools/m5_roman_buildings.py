#!/usr/bin/env python3
"""Render and validate named AD 1 Roman civic-building specials.
...
def load() -> list[dict[str, str]]:
    ...
    for number, row in enumerate(items, start=2):
        ...
        if row["location"] != "rome":
            failures.append(f"{prefix}: Roman civic special must use installed location rome")
    ...
    for key, row in special.items():
        ...
        if row["location"].strip() != expected["location"] or not row["key"].strip().startswith("roma_"):
            raise ValueError(f"special_buildings.csv entry for {key} must use its ledger location and a Roma-prefixed start key")
    return items
...
def definition(items: list[dict[str, str]]) -> str:
    lines = [
        ...
        "# Named, one-level Roman civic specials; historical ledger: docs/m5/roman_buildings.csv.",
        "",
    ]
    for row in items:
...
def loc(items: list[dict[str, str]], language: str) -> str:
    lines = [f"l_{language}:", " # Generated named AD 1 Roman civic-building localization; English mirrored by design."]
    for row in items:
