...
def r5_root_overrides() -> dict[str, str]:
    if not R5_GEOGRAPHY.is_file():
        return {}
    result: dict[str, str] = {}
    with R5_GEOGRAPHY.open(encoding="utf-8-sig", newline="") as handle:
...
def localization(entries: tuple[Correction, ...], client_language: str) -> str:
    lines = [
        ...
        " # Generated by tools/generate_m4_location_name_corrections.py from docs/m4/location_name_corrections.csv.",
        " # Authoritative late-loading corrections; English is mirrored by design. Do not edit this file directly.",
    ]
    overrides = r5_root_overrides()
    for entry in entries:
        name = esc(overrides.get(entry.location, entry.historical_name))
