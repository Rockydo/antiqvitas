...
EVENT_IMAGES = {
    ...
    "champa_formation": "gfx/interface/illustrations/event/antq_champa_formation.dds",
    "daqin_embassy": "gfx/interface/illustrations/event/antq_daqin_embassy.dds",
    "five_emperors": "gfx/interface/illustrations/event/antq_five_emperors.dds",
    ...
    "moche_formation": "gfx/interface/illustrations/event/antq_moche_formation.dds",
...
def validate(records: tuple[Current, ...]) -> None:
    ...
    if MOCHE_TAG in mapped_tags.values():
        raise ValueError(f"M10 dynamic tag {MOCHE_TAG} collides with the AD 1 tag map")
    ...
    viru = mapped_tags.get("VIR")
    if viru is None:
        raise ValueError("M10 Moche formation requires the AD 1 Viru/Gallinazo proxy")
    if MOCHE_SEED not in start_country_locations(viru):
        raise ValueError(f"M10 Moche seed {MOCHE_SEED} is not an AD 1 Viru/Gallinazo holding")
...
def moche_formation_lines() -> tuple[str, ...]:
    """Render the plan's circa-AD-100 Moche emergence from the AD 1 proxy.

    The formation deliberately creates a new country rather than cosmetically
    renaming the Gallinazo holder. That preserves the historically bounded AD
    1 archaeology while making the later Moche current visible to the engine.
    """
    return (
        "\t\t# P8.10; JSTOR-MOC; CAM-MOC; coarse north-coast local-mesh proxy.",
        f"\t\tlocation:{MOCHE_SEED} = {{",
        "\t\t\tcreate_country_from_location = {",
        f"\t\t\t\tdefine_unique_country_tag = {MOCHE_TAG}",
        f"\t\t\t\tchange_country_name = {MOCHE_TAG}",
        f"\t\t\t\tchange_country_adjective = {MOCHE_TAG}",
        f"\t\t\t\tchange_country_color = map_{MOCHE_TAG}",
        f"\t\t\t\tchange_country_flag = {MOCHE_TAG}",
        "\t\t\t\tchange_culture = culture:antq_moche",
        "\t\t\t\tchange_religion = religion:antq_andean",
        "\t\t\t\tchange_government_type = government_type:monarchy",
        "\t\t\t\tadd_reform = government_reform:antq_regional_kingship",
        "\t\t\t\tchange_heir_selection = heir_selection:cognatic_primogeniture",
        "\t\t\t}",
        f"\t\t\tadd_core = c:{MOCHE_TAG}",
        "\t\t}",
        "\t\tadd_prestige = prestige_mild_bonus",
    )
...
def impact_lines(record: Current) -> tuple[str, ...]:
    ...
    if record.key == "antonine_plague":
        ...
        return (
            ...
            "\t\tadd_gold = { value = root.monthly_income_trade_and_tax multiply = -4 }",
            "\t\t# SCI-ANT: Antioch is an eastern-circuit anchor, not a claimed first case.",
            "\t\tlocation:antioch = {",
            "\t\t\tspawn_disease = {",
            ...
            "\t\t\t# A separate outbreak proxy avoids asserting one intercontinental chain.",
            "\t\t\tlocation:luoyang = {",
            "\t\t\t\tspawn_disease = {",
            "\t\t\t\t\tdisease = disease:smallpox",
            "\t\t\t\t\tvalue = 0.20",
            "\t\t\t\t}",
            "\t\t\t\tprovince = { change_province_food_percentage = -0.08 }",
            "\t\t\t}",
            "\t\t}",
        )
    if record.kind == "disaster":
...
def colors() -> str:
    return "\n".join((
        "# Generated by tools/m10_second_century.py --write; temporary M10 formation colors.",
        "colors = {",
        "\tmap_CPC = rgb { 175 103 48 }",
        "\tmap_MOC = rgb { 167 97 45 }",
        "}",
        "",
    ))
...
def coas() -> str:
    return "\n".join((
        "# Generated by tools/m10_second_century.py --write; M11 non-reconstructive formation UI standard.",
        "CPC = {",
        ...
        "\tcolor2 = \"yellow\"",
        "\tcolor3 = \"black\"",
        "\tcolored_emblem = { texture = \"ce_andean_small_bird.dds\" color1 = color2 color2 = color2 color3 = color3 instance = { position = { 0.5 0.5 } scale = { 0.82 0.82 } } }",
        "}",
        "",
    ))
