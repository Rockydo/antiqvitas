...
under the host's scaled geometry. Fresh Roman and Parthian Government panels
confirm the date-less current-term contract as well as their intended
government/law profiles, and the Roman player-panel probe also confirms all
five source-labelled estate adapters. Han has a separate, reproducible silent
minority-regency fallback even with two current-term variants and therefore
blocks M6 acceptance; see `BLOCKERS.md` and
`docs/playtests/M6_CORE_FOUNDATION.md`.
