...
- [ ] Complete runtime pacing/growth/inflation balance from observer measurements (the clean Vulkan run measured 0.316792 simulated days/second through 18 March AD 1 before the renderer crash; native DX12 reached 3 March but reproduced the same FSR/NGX exit, so annual growth/inflation and long-run AI remain blocked in `BLOCKERS.md`).
- [ ] Run autonomous observer game to 476 with decade screenshots and live log watch (event-graph coverage is green; three Vulkan and one native-DX12 profile reproduce the FSR/NGX renderer crash, and the player-context legacy-scope audit remains deferred in `BLOCKERS.md`).
...
- [ ] [BLOCKED] Tag `M12-done`: observer pacing, the AD 1-to-476 run, and screenshot evidence remain unavailable after the documented Vulkan and native-DX12 FSR/NGX renderer reproductions in `BLOCKERS.md`.
