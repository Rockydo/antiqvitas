...
mod localization. It emits 28,573 transparent root fallbacks and 11,437
culture-bound populated-field adapters after higher-confidence names are
excluded. Tier-3 keeps the installed cartographic label; it never invents a
pseudo-linguistic `-haim`, `-dun`, or equivalent form.
...
Map-only keys without an installed English label receive a human-readable
location-key fallback. `T3N` marks these as unresolved cartographic labels,
not historical assertions.

`tools/m4_frontier_language_names.py` adds 140 Pleiades-derived Latin viewing
aliases across Germania, Britain, Gaul, and the Danube-Balkan frontier.
Eighteen overt Roman administrative forms retain a separate local label.
