...
- **Milestone gates:** each milestone in §22 ends with `make full` green **plus** a short, targeted driver probe for the subsystem changed (map mode, one screen, a dated trigger, or a bounded 7–30 in-game-day burn-in). Write the findings — with screenshots where useful — to `docs/playtests/M<N>_REPORT.md`, judging pass/fail against the milestone's acceptance criteria. Static chronology, symbol, date, and asset checks carry the broad coverage; an exhaustive observer playthrough is not a completion requirement. If the driver can't perform a step, note the exact gap in the report and in KNOWN_ISSUES.md, keep the log-diff bar mandatory, and move on: **the project never blocks on a human.**
...
- **Image generation:** for event illustrations (~80 shared paintings), 5 age splashes, 8–10 loading screens, situation/disaster art, and player-facing icons. Global style prompt: *"muted painterly historical illustration in the style of Roman fresco and Han dynasty mural art, soft natural light, no text, no borders, no modern objects"* + subject; per-class negative prompts; upscale/crop to exact vanilla dimensions; convert with `tools/dds.py`; visually spot-check by rendering a contact-sheet HTML and reviewing it yourself (screenshot it into the milestone report; it's also a nice artifact for the user to browse later).
- **Direct visual coverage:** every player-facing ANTIQVITAS advance, privilege, building, good, religion, and institution receives a dedicated, subject-specific illustration/icon and a checked DDS contract. Shared art is permitted only for event illustrations explicitly designed as a shared scene; it is not a fallback for UI objects.
...
**M5 — Economy.** Goods roster + RGO remap; markets; buildings + town_setups; road_network seeded (Roman roads from ORBIS/itineraries; Persian royal road legacy; Chinese trunk roads); development pass. Rome receives a dense, source-led civic and commercial building layer: public grain storage, market, forum/basilica administration, water, bathing, mills/bakeries, workshops, mints, harbour/river commerce, entertainment, cult, villas, military supply, and frontier infrastructure, each mapped only through locally verified engine building contracts and tied where possible to an appropriate ancient good or service. *Accept:* source-led building ledger and direct UI art are complete; rapid market/building probes show the key Roman nodes load and apply; trade-flow claims require positive evidence but no extreme observer run.
...
**M7 — War.** Units, levy/regular assignments, mercs, forts/limes, navies; prune gunpowder+oceanics. *Accept:* a bounded, controlled driver war probe shows declaration, battle resolution, and the relevant army/fort screens without errors.

**M8 — Knowledge.** Full 5-age advance trees (~250), institutions, starting tech tiers, age objectives/abilities. *Accept:* static unlock/dead-end/chronology checks pass and a short targeted research-screen probe shows the starting tree coherently; no long AI research campaign is required.
...
**M10 — History in motion.** All §9 situations/disasters/tag-switches/formables, in chronological batches (1–100, 100–200, 200–300, 300–400, 400–476), each batch statically checked for dates, recipients, symbols, localization, and art, then smoke-tested with a short targeted driver probe where the engine can reach it promptly. *Accept:* all historical-current contracts validate and targeted probes have no mod-only diagnostics; a centuries-long observer simulation is not required.

**M11 — Flavor & face.** Events/decisions to §18 targets; final flags; dedicated advance/privilege/building/good/religion/institution icons; event art; loading screens; age splashes; period-name polish; concept glossary; credits. *Accept:* no placeholder or shared-fallback art remains on any player-facing ANTIQVITAS UI object, and text/location names use the appropriate AD 1 period form.

**M12 — Ship.** Balance/pacing pass (advance costs, pop growth, inflation), AI weights sanity, fresh-start and targeted subsystem probes, KNOWN_ISSUES.md, README, thumbnail final, PDX Workshop Manager packaging notes. *Accept:* `make full` green; fresh AD 1 start is clean; every high-risk subsystem has a passing rapid probe or a documented engine limitation; the finale is statically date/symbol/localization validated. A full AD 1-to-476 observer game is optional evidence, not a ship blocker.
...
- **R5 Icon volume (~250 advances) overwhelms art budget** → staged, templated generation with a subject ledger and contact-sheet review; each player-facing object retains its own dedicated final asset.
